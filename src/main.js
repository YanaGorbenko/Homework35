...
buttonAllBooks.className = 'all-books-button';
buttonAllBooks.textContent = 'Показати всі книги';
listContainer.prepend(buttonAllBooks);
buttonAllBooks.style.display = 'none';

const select = document.createElement('select');
listContainer.prepend(select);
select.style.display = 'none';

const option1 = document.createElement('option');
option1.value = 'asc';
option1.textContent = 'В алфавітному порядку';
select.appendChild(option1);

const option2 = document.createElement('option');
option2.value = 'desc';
option2.textContent = 'Проти алфавітного порядку';
select.appendChild(option2);

const searchForm = document.createElement('form');
searchForm.className = 'searchForm';
listContainer.prepend(searchForm);
...
async function renderBookList(books) {
  books.forEach(book => {
    ...
    } else if (books.length < 5 && currentPage !== 1) {
      listEndMessage();
    }
...
buttonAllBooks.addEventListener('click', async () => {
  bookList.innerHTML = '';
  currentPage = 1;
  search = '';
  await getBooks();
